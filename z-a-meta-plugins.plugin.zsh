...
zi_annex_meta_plugins_config_map=(
  ...
  # @z-shell
  z-shell/F-Sy-H                      "$_std atinit'ZI[COMPINIT_OPTS]=-C; zicompinit; zicdreplay;' atload'FAST_HIGHLIGHT[chroma-man]=; fast-theme z-shell &>/dev/null;' atclone'(){local f;cd -q →*;for f (*~*.zwc){zcompile -Uz -- ${f}};}' compile'functions/.*fast*~*.zwc'"
